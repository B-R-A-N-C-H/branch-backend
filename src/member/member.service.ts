...
export class MemberService {
    ...
    memberHasRole(member: JwtPayload, role: Role = Role.TEACHER) {
        if (!member)
            return false;
        if (member.sub.role === Role.ADMIN || member.sub.role === Role.PRINCIPAL)
            return true;
        if (member.sub.role === Role.HEAD_TEACHER && role === Role.TEACHER)
            return true;
        return member.sub.role === role;
    }
